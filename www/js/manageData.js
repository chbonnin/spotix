...
//Initialisation
function initData(){
    if (localStorage.coefSize)
	coefSize=parseFloat(localStorage.coefSize);

    MarkerIcon = L.Icon.extend({//Marqueur de base d'où sont dérivés les autres marqueurs
                               options: {
                               shadowUrl: 'img/icon/ombre.png',
                               iconSize:     [66*coefSize, 87*coefSize],
                               shadowSize:   [41*coefSize, 41*coefSize],
                               iconAnchor:   [32*coefSize, 86*coefSize],
                               shadowAnchor: [14*coefSize, 40*coefSize],
                               popupAnchor:  [0, -85*coefSize]                               }
                               });
...
function initMarkers(){
    ...
    for (iSpot=0;iSpot<lstSpot.length; iSpot++){
        ...
        lstSpot[iSpot].marker=L.marker([lstSpot[iSpot].latitude, lstSpot[iSpot].longitude],
                                       {icon:lstCat[iCat].icon, zIndexOffset: (iCat==1 ? 0 : 10)}).bindPopup(lstSpot[iSpot].htmlTooltip());
